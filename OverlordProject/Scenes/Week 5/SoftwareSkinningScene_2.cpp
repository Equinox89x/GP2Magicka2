...
void SoftwareSkinningScene_2::Update()
{
	...
	for (size_t i = 0; i < m_SkinnedVertices.size(); i++)
	{
		...
		bool result{ i < 24 };
		auto var{ result ? m_pBone0->GetBindPose() : m_pBone1->GetBindPose() };
		XMMATRIX world{ result ? XMLoadFloat4x4(&m_pBone0->GetTransform()->GetWorld()) : XMLoadFloat4x4(&m_pBone1->GetTransform()->GetWorld()) };
		...
		m_SkinnedVertices[i].transfornedVertex.Position = pos;
	}
